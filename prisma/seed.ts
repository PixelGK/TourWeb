...
async function main() {
  ...
  for (const mockTour of toursToSeed) {
    ...
    for (const variant of getMockVariants(mockTour.slug)) {
      const supplierUnitCostIdr = variant.code === "standard-solo" ? 275000 : variant.code === "standard-tandem" ? 375000 : variant.code === "premium-solo" ? 500000 : 750000;
      const remainderCostIdr = variant.code.startsWith("standard") ? 275000 : 500000;
      await prisma.tourVariant.upsert({
        where: { tourId_code: { tourId: tour.id, code: variant.code } },
        update: { ...variant, supplierUnitCostIdr, remainderCostIdr, active: true },
        create: { ...variant, tourId: tour.id, supplierUnitCostIdr, remainderCostIdr, active: true },
      });
    }
