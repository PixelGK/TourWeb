...
function BookingEmailStatus({ status, confirmed, requestEmailSent, confirmationEmailSent }: { status: string; confirmed: boolean; requestEmailSent: boolean; confirmationEmailSent: boolean }) {
  if (["CANCELLED", "REFUNDED"].includes(status)) return <span className="block text-[0.65rem] font-semibold uppercase tracking-wide text-weathered">No email action</span>;
  if (confirmed) return <span className={`block text-[0.65rem] font-semibold uppercase tracking-wide ${confirmationEmailSent ? "text-success" : "text-error"}`}>{confirmationEmailSent ? "Confirmation emailed" : "Confirmation email pending"}</span>;
  if (status === "REQUESTED") return <span className={`block text-[0.65rem] font-semibold uppercase tracking-wide ${requestEmailSent ? "text-success" : "text-error"}`}>{requestEmailSent ? "Request emailed" : "Request email pending"}</span>;
  return <span className="block text-[0.65rem] font-semibold uppercase tracking-wide text-weathered">Payment email follows provider status</span>;
}
