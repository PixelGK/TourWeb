...
export default async function AdminBookingsPage({ searchParams }: { searchParams: Promise<{ status?: string; tour?: string; q?: string }> }) {
  ...
  return (
    <div className="space-y-8">
      <AdminPageHeader eyebrow="04 · Guest ledger" title="Bookings" description="Find a guest quickly, confirm the provider’s status, or cancel a pending reservation and release its seats." />
      <form className="grid gap-3 border border-charcoal/30 bg-frangipani p-4 sm:grid-cols-[1fr_0.7fr_0.9fr_auto] sm:items-end">
        ...
        <label className="text-sm font-semibold">Status<select name="status" defaultValue={filters.status ?? "ALL"} className="mt-2 min-h-11 w-full border border-charcoal/35 bg-limestone px-3 font-normal"><option value="ALL">All statuses</option>{["PENDING", "PAID", "CANCELLED", "REFUNDED"].map((status) => <option key={status}>{status}</option>)}</select></label>
      ...
      <div className="overflow-x-auto border border-charcoal/25 bg-frangipani">
        <table className="w-full min-w-[68rem] text-left text-sm">
          ...
          <tbody className="divide-y divide-charcoal/15">{bookings.map((booking) => <tr key={booking.id}><td className="px-4 py-4"><strong className="block">{booking.customerName}</strong><span className="block text-xs text-weathered">{booking.reference}</span><a href={`mailto:${booking.customerEmail}`} className="block max-w-52 truncate text-xs text-terrace hover:underline">{booking.customerEmail}</a></td><td className="px-4 py-4"><span className="block max-w-64 truncate">{booking.tourTitle}</span><time className="text-xs text-weathered" dateTime={booking.date}>{booking.date}</time></td><td className="px-4 py-4 tabular-nums">{booking.paxCount} pax</td><td className="space-y-1 px-4 py-4"><AdminStatus status={booking.status} /><span className="block text-[0.65rem] uppercase tracking-wide text-weathered">Provider: {booking.paymentStatus.replaceAll("_", " ")}</span></td><td className="px-4 py-4 font-semibold tabular-nums">{idr.format(booking.totalAmountIdr)}</td><td className="px-4 py-4"><BookingActions id={booking.id} status={booking.status} preview={session.preview} /></td></tr>)}</tbody>
        </table>
        {bookings.length === 0 ? <p className="p-8 text-center text-sm text-weathered">No bookings match those filters.</p> : null}
      </div>
      <p className="border-l-4 border-gold pl-4 text-sm leading-6 text-weathered"><strong className="text-charcoal">Why there is no “Mark paid” button:</strong> payment state only changes after a verified Midtrans webhook or a server-to-server status recheck. An admin click cannot invent a successful payment.</p>
    </div>
  );
}
