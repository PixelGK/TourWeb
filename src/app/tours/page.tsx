...
export default async function ToursPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const results = filterTours(filters);
  ...
  const activeFilterCount = [filters.category, filters.duration, filters.price].filter(Boolean).length;
          ...
          <section aria-labelledby="results-heading">
            <div className="flex flex-col gap-5 border-b border-charcoal/25 pb-6 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h2 id="results-heading" className="mt-2 font-serif text-3xl text-charcoal">{results.length} {results.length === 1 ? "tour" : "tours"} found</h2>
              </div>
