import { ArrowLeft, Check, Clock3, ExternalLink, MapPin, Users, X } from "lucide-react";
  ...
  return (
    <main className="bg-[#fbfaf6] pb-36 lg:pb-0">
      ...
      <section className="border-b border-charcoal/15 bg-[#fbfaf6]">
        <div className="site-shell max-w-[90rem] pb-10 pt-8 lg:pb-14 lg:pt-10">
          <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-2 text-xs font-semibold text-weathered">
            <Link href="/tours" className="inline-flex items-center gap-1.5 hover:text-terrace"><ArrowLeft aria-hidden="true" className="size-3.5" /> All tours</Link>
            <span aria-hidden="true">/</span><span>{tour.category}</span>
          </nav>

          <div className="mt-8 grid gap-9 lg:grid-cols-12 lg:items-end">
            <div className="lg:col-span-8">
              <p className="text-sm font-semibold text-clay">{tour.category} · private vehicle</p>
              <h1 className="mt-4 max-w-4xl font-serif text-5xl font-normal leading-[0.94] tracking-[-0.035em] text-terrace sm:text-6xl">{tour.title}</h1>
              <p className="mt-6 max-w-3xl text-lg leading-8 text-charcoal/68">{tour.summary}</p>
            </div>
            <div className="grid gap-3 border-t border-charcoal/25 pt-5 text-sm font-semibold text-charcoal/75 lg:col-span-4 lg:max-w-sm lg:justify-self-end">
              <span className="inline-flex items-center gap-2"><Clock3 aria-hidden="true" className="size-4 text-clay" /> {tour.duration}</span>
              <span className="inline-flex items-center gap-2"><Users aria-hidden="true" className="size-4 text-clay" /> {isExperienceDay ? `Private experience day · up to ${tour.maxGroupSize}` : `Private vehicle · up to ${tour.maxGroupSize}`}</span>
      ...
      <div className="site-shell max-w-[90rem] grid gap-12 py-14 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-start lg:py-20">
        <div className="min-w-0 space-y-16">
          <DayPlanPromise compact />

          <section aria-labelledby="itinerary-heading">
            <h2 id="itinerary-heading" className="font-serif text-4xl font-normal leading-none sm:text-5xl">What happens on the day</h2>
          ...
          <section className="grid gap-8 md:grid-cols-2" aria-labelledby="included-heading">
            <div className="border-t border-charcoal/30 py-6">
              <h2 id="included-heading" className="font-serif text-3xl font-normal">What’s included</h2>
            ...
            <div className="border-t border-charcoal/30 py-6">
              <h2 className="font-serif text-3xl font-normal">Not included</h2>
              <ul className="mt-5 space-y-3">
          ...
          <section aria-labelledby="pricing-heading">
            <h2 id="pricing-heading" className="font-serif text-4xl font-normal sm:text-5xl">{tour.pricingMode === "PER_VEHICLE" ? "One price for the vehicle" : isExperienceDay ? "Complete day price per person" : "Price per person"}</h2>
          ...
          <section className="grid gap-8 md:grid-cols-2" aria-label="Meeting and cancellation details">
            <div className="border-t border-charcoal/30 py-6">
              <h2 className="font-serif text-3xl font-normal">Meeting point</h2>
              <p className="mt-4 font-semibold">{tour.meetingPoint}</p>
              <p className="mt-2 text-sm leading-6 text-weathered">{tour.meetingNote}</p>
              <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(tour.location + ", Bali")}`} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-terrace hover:underline">View the tour area <ExternalLink aria-hidden="true" className="size-3.5" /></a>
            </div>
            <div className="border-t border-charcoal/30 py-6">
              <h2 className="font-serif text-3xl font-normal">Cancellation policy</h2>
          ...
          <section className="border-t border-charcoal/25 pt-8" aria-labelledby="reviews-heading">
            <h2 id="reviews-heading" className="font-serif text-4xl font-normal sm:text-5xl">Guest reviews are coming</h2>
