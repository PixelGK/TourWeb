...
export function BookingWidget({ tourSlug, pricingTiers, maxGroupSize, blackoutDates = [], initialDate = "", initialPax = 2 }: { tourSlug: string; pricingTiers: PricingTier[]; maxGroupSize: number; blackoutDates?: string[]; initialDate?: string; initialPax?: number }) {
  ...
  return (
    <>
      <aside className="sticky top-6 hidden border border-charcoal/25 bg-frangipani p-6 shadow-sun-raised lg:block" aria-label="Book this tour">
        ...
        <BookingForm tourSlug={tourSlug} pax={pax} setPax={setPax} maxGroupSize={maxGroupSize} minDate={bookingWindow.minDate} maxDate={bookingWindow.maxDate} blackoutDates={blackoutDates} initialDate={initialDate} />
...
function BookingForm({ tourSlug, pax, setPax, maxGroupSize, minDate, maxDate, blackoutDates, initialDate }: { tourSlug: string; pax: number; setPax: (pax: number) => void; maxGroupSize: number; minDate: string; maxDate: string; blackoutDates: string[]; initialDate: string }) {
  return (
