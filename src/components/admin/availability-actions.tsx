...
export function AvailabilityRangeForm({ tours, preview }: { tours: AdminTourRow[]; preview: boolean }) {
  ...
  return (
    <form action={action} className="border border-charcoal/30 bg-frangipani p-5 shadow-sun sm:p-6">
      <fieldset disabled={preview || pending} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Select label="Tour" name="tourId" required containerClassName="sm:col-span-2 lg:col-span-1">{tours.map((tour) => <option key={tour.id} value={tour.id}>{tour.title}</option>)}</Select>
        ...
        <div className="sm:col-span-2 lg:col-span-4 flex flex-col gap-3 border-t border-charcoal/15 pt-4 sm:flex-row sm:items-center sm:justify-between">
